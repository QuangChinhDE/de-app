...
export function FilterForm({ schema, value, onChange, onRun, isRunning, stepOutputs = {} }: NodeFormProps<FilterNodeConfig>): JSX.Element {
  const {
    handleSubmit,
    watch,
    setValue,
    formState: { errors },
  } = useForm<FilterNodeConfig>({
  ...
  const onSubmit = handleSubmit((data: FilterNodeConfig) => {
    onRun(data);
  });
      ...
      <div className="flex-1 space-y-6 overflow-y-auto px-4 py-4">
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-ink-700">
            Filter Mode
            <span className="ml-2 text-xs font-normal text-ink-500">
              (Include: keep matching | Exclude: remove matching)
            </span>
          </label>
          <div className="flex gap-3">
            <button
              ...
              className={`flex-1 rounded-lg border-2 px-4 py-3 text-sm font-semibold transition-all ${
                mode === "include"
                  ...
                  : "border-ink-200 bg-white text-ink-600 hover:border-emerald-300 hover:bg-emerald-50"
              }`}
            >
              ✓ Include
            </button>
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-ink-700">
            Filter Conditions
            <span className="ml-2 text-xs font-normal text-ink-500">
              (Define rules to filter array items)
            </span>
          </label>
